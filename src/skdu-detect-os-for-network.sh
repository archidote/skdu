...
if [ "`cat /etc/issue | cut -c1-6`" == "Ubuntu" ] || [ "`cat /etc/issue | cut -c1-13`" == "elementary OS" ]; then
    ...
    /etc/skdu/src/skdu-ip-netplan.sh # on précise le dossier src du projet de base
elif [ "`cat /etc/issue | cut -c1-6`" == "Debian" ]; then
    echo -e "\e[92m Distribution : Debian \e[0m "
    /etc/skdu/src/skdu-ip-debian.sh # on précise le dossier src du projet de base
    ...
    echo -e " nb : Compatible OS : Debian, Ubuntu, Elementary OS, Kalilinux"
fi
